...
<template>
  <section class="rounded-2xl border border-border/70 bg-card/60 p-5">
    ...
    <div v-else class="grid gap-2.5">
      <article v-for="row in rows" :key="row.label" class="flex items-center gap-3 rounded-lg py-1">
        ...
        <strong class="w-20 text-right font-mono text-xs font-bold text-foreground" style="font-variant-numeric: tabular-nums">
