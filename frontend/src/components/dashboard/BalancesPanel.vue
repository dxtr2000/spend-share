...
<template>
  <section class="min-w-0 rounded-2xl border border-border/70 bg-card/60 p-5">
    ...
    <div class="grid min-w-0 gap-2.5">
      <article v-for="row in rows" :key="row.member.id" class="flex min-w-0 items-center gap-3">
        <span
        ...
        <div class="relative h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-background/70">
          <span
        ...
        <strong
          class="w-16 shrink-0 truncate text-right font-mono text-xs font-bold"
